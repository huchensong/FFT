...
void stage_routine(int8_t stage,int16_t fft_length,int8_t base,int16_t* in_buf,int16_t* out_buf){
    ...
    cout<<" base ="<<fft_length<<" base ="<<base<<" 2^stage = "<<pow(2,stage);
    ...
    for(int k=0; k<block_num;k++){
        for(int cnt =0;cnt<block_cnt;cnt++){
            ...
            {
                case 2:
                    base2(base_addr + cnt,base_addr + cnt + data_gap,calculate_w(1,k,N),in_buf);
...
int main(){
    ...
    int fft_length=0;
    while(infile)infile>>in_data[fft_length++];
    fft_length-=1;
    #ifdef print_in_out  
    ...
    int8_t base    = 2;
    int8_t in_type = 1;
    fft_hcs(in_data,out_result,fft_length,base,in_type);
    #ifdef print_in_out  
    for(int i=0;i<fft_length;i++)cout<<in_data[i]<<' ';
    #endif
} 